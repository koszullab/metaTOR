...
def extract_contigs(assembly, list_contigs, output_file):
    ...
    list_contigs : list of str
        List with the names used in the assembly file as contigs.
    output_file : str
...
def generate_fasta(assembly, communities, contigs_data, output_dir):
    ...
    communities : dict
    contigs_data : str
        Path to the file of the contigs
    output_dir : str
        Path to the output where the fasta will be written.
...
def louvain_iterations_py(network_file, iterations, output_dir):
...
def hamming_distance(core_communities_iterations, output, n_iter, threads):
    """Generate matrix of Hamming distances between all pairs of core
    communities

    Parameters
    ----------
    core_communities_iterations : dict
        Iterative clustering matrix, obtained by pasting all Louvain
        clusterings.
    core_communities : dict
        Contig ids of core communities. (last column of
        partition/core_size_indices_*.txt)
    n_iter : int
        Number of iterations of Louvain.
    output : str
        Output file to store matrix of Hamming distances.
    threads : int
    ...
    logger.info("Merging sub-matrices...")
...
def defined_overlapping_communities(
    ...
    -----------
    overlap : int
    hamming_distance : array
    core_communities : dict
    core_communities_iteration : pandas.dataframe
    ...
    # distance smaller than the threshold given.
