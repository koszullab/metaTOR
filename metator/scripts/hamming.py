...
def get_distances_splitmat(comm, core_communities_contigs):
    ...
    x = sparse.csr_matrix(
        ...
        - metrics.pairwise_distances(
            core_communities_contigs, comm.values, metric="hamming"
        )
    )
    return x
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generate matrix of Hamming distances between all pairs of \
            core communities"
    )
    parser.add_argument(
        "-c",
        "--communities",
        help="Iterative clustering matrix, obtained by pasting all Louvain \
            clusterings",
    )
    parser.add_argument(
        "-i",
        "--indices",
        help="Contig ids of core communities (last column of \
            partition/core_size_indices_*.txt)",
    )
    parser.add_argument(
        "-C",
        "--contigs",
        help="<PARTITION_DIR>/network_matrices/<PROJECT>_<ITER>-iters_contigID-coreID-contigSize.txt",
    )
    parser.add_argument(
        "-o",
        "--output",
        help="Output file to store matrix of Hamming distances",
    )
    parser.add_argument(
        "-@",
        "--cores",
        help="Number of cores to parallelize computation",
        default=5,
    )
    ...
    core_communities = core_communities.apply(
        lambda x: [int(y) for y in x[0].split(" ")], axis=1
    )
    core_communities.index = [
        "cc_" + str(x + 1) for x in range(0, len(core_communities))
    ]

    # ------- Create core-community-level iterative clustering matrix ------- #
    core_communities_contigs = communities.loc[
        np.array(core_communities.apply(lambda x: "contig_" + str(x[0]))),
    ]
    ...
    split_core_communities = [
        core_communities_contigs[(k - step) : k] for k in steps
    ]
    pool = multiprocessing.Pool(processes=cores)
    res = pool.map(
        partial(
            get_distances_splitmat,
            core_communities_contigs=core_communities_contigs,
        ),
        split_core_communities,
    )
    ...
    cc_sizes = [
        sum(contigs.iloc[np.array(cc) - 1, 2]) for cc in core_communities
    ]
    ...
    output2 = output.replace(
        "_hamming_distance.npz", "_ccID_ccSize_ccNbContigs.txt"
    )
    np.savetxt(
        output2,
        core_communities[["idx", "size", "NbContigs"]].values,
        fmt="%d",
        delimiter="\t",
    )
