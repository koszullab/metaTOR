...
if [ ! -f $assembly_dir/${project}_assembly_raw.fa ]; then
  cp $assembly $assembly_dir/${project}_assembly_raw.fa || { echo >&2 "I can't find fasta file to map against. Aborting."; exit 1; }
...
if [ $minimap2 -eq 0 ]; then
  if [ ! -f ${index_name}.1.bt2 ]; then
    ...
    || { echo >&2 "Something wrong happened when building index. Aborting."; exit 1; }
    echo "Done."
  fi
fi
