...
global_args = {
    ...
    "CONTIGS" : "tests_data/outdir/idx_contig_length_GC_hit_cov.txt",
...
def test_align():
    args = ("-1 {FASTQ_FOR} -2 {FASTQ_REV} -g {FASTA_INDEX} -o {OUT} -q 30 -t 8 -T {TMP}").format(**global_args)
    proc = mtc.Align(args.split(" "), {})
    proc.execute()

@pytest.mark.parametrize("mode", ["for_vs_rev", "all", "pile"])
def test_cutsite(mode):
    args = ("-1 {FASTQ_FOR} -2 {FASTQ_REV} -e DpnII,HinfI -m {0} -o {OUT} -t 8").format(mode, **global_args)
    proc = mtc.Cutsite(args.split(" "), {})
    proc.execute()


def test_network():
    args = ("-g {FASTA} -i {ALIGNMENT} -n -o {OUT} -t 8 -T {TMP}").format(**global_args)
    proc = mtc.Network(args.split(" "), {})
    proc.execute() 


def test_partition():
    args = ("-a {FASTA} -c {CONTIGS} -i 5 -n {NETWORK} -o {OUT} -O 90 -s 30000 -t 8 -T {TMP}").format(**global_args)
    proc = mtc.Partition(args.split(" "), {})
    proc.execute()
    

def test_pipeline():
    args = ("-1 {FASTQ_FOR} -2 {FASTQ_REV} -g {FASTA_INDEX} -i 5 -n -o {OUT} -O 90 -q 30 -s 30000 -t 8 -T {TMP}").format(**global_args)
